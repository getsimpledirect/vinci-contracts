...
### Contract-gap status for 0.2.0

1. **Closed in 0.1.0.** `AuthorityCommandEnvelope`, `validateAuthorityCommandEnvelope`, and `authorityCommandSigningPayload` contract the signed request; `AuthorityResultEnvelope` and `validateAuthorityResultEnvelope` contract the host result; and the `authority.acknowledged` / `authority.rejected` members of `RUN_EVENT_TYPES` provide the durable ACK/REJECT record.
2. **Closed in 0.1.0.** `SessionBindingRef`, `validateSessionBindingRef`, and `frameMatchesBinding` make literal connection binding checkable; schema-v2 `SessionFrame` carries organization/workspace identity; and `ReplayGap` with `validateReplayGap` contracts the replay control result.
3. **Closed in 0.1.0.** `RUN_EVENT_TYPES`, `PAYLOAD_FIELDS`, and `validateRunEvent` now cover `device.revoked`, relay unavailable/restored, host unreachable/reachable, and authority acknowledged/rejected with closed, content-minimal payloads.
4. **Closed in 0.2.0.** `RelayAccessToken` remains the signed relay credential; `RelayAccessTokenRequest` contracts device-to-Platform minting; `KeyDirectoryResponse` resolves and rotates issuer/device/host verification keys; and signed, monotonic `RevocationSnapshot` supports the 15-second pull plus immediate push paths.
5. **Still open in 0.2.0.** The mandatory end-to-end key lifecycle, multi-device rotation/recovery rules, and durable-event encryption boundary still require a Security/Platform Data decision.
6. **Still open in 0.2.0.** Infrastructure/Compliance has not yet approved an initial relay region or disaster-recovery residency boundary.
