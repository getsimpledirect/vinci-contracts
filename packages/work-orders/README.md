...
`schemaVersion: 1`, `workOrderId`, `workOrderDigest`, `repository {host, owner, name}`, `baseRef` and `targetBranch` (plain branch names — `main`, never `refs/heads/main` — under the rules the worker adopted in vinci-code-cli PR #8 plus `git check-ref-format --branch`: alphanumeric first character, then `[A-Za-z0-9._/-]`, no `..`/`//`/`refs/`/`.lock`/`@{`/`~`/`^`/`:`/leading `-` or `+`, not `HEAD`; exported as `isPlainBranchName`), `baseCommit` (exactly 40 **lowercase** hex characters — uppercase is rejected by policy so commits compare bytewise, the same rule `isDigest` applies to SHA-256), `targetBranch`, `modelClass`, optional `provider`, `resourceBounds {budgetMicrousd, maxRuntimeS, deadline}`, `tools[]`, `inputArtifacts [{id, digest}]`, `requiredCapabilities[]`, `output` (`branch` | `patch` | `artifact` | `none`), `evidence {required}`, `promotion` (`pull_request` | `none`), `issuedAt`. Unknown fields are rejected; a spec is immutable once issued — a changed commit or bound is a new spec with a new digest.
